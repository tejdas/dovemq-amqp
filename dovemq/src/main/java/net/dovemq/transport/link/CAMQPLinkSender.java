...
class CAMQPLinkSender extends CAMQPLinkEndpoint implements CAMQPLinkSenderInterface, Runnable {
     ...
     * for this link. A non-zero value is an indication that session is under
     * flow-control. Incremented before call to
     * CAMQPSessionInterface.sendTransfer() Decremented in the callback from
     * session layer: messageSent() Flow-control calculation in the link layer
     * takes this value into account. If linkCredit <= unsentMessagesAtSession,
     * the link layer is under flow-control.
     */
    ...
    /**
     * Remembers when the next flow frame needs to be explicitly sent, asking
     * for more link credit.
     */
    private long nextCreditRequestTime = -1;

    /**
     * Helps sending subsequent flow-frames with exponential back-off period.
     */
     ...
     * Processes the flow frame: (a) updates the link credit, taking into
     * account the delta between Link Sender's deliveryCount and Link Receiver's
     * delivery count. (b) If there are unsent messages waiting for link credit
     * to become available and there is available link credit (after it was
     * updated), then executes a Runnable to start sending messages. (c) If
     * there is drain requested or echo flow requested, then process and sends a
     * flow frame.
     */
    @Override
    public void flowReceived(CAMQPControlFlow flow) {
        ...
        synchronized (this) {
            ...
            if (!receivedFirstFlowCredit && (linkCredit > 0)) {
                /*
                 * Received the first flow-frame with non-zero link credit.
                 * Notify the thread waiting for link credit after the link
                 * establishment.
                 */
            ...
            /*
             * Reset nextCreditRequestTime to -1, so that flow-frames asking for
             * link-credit are not scheduled any more.
             */
            if (linkCredit > 0) {
    ...
    /**
     * Called by {@link CAMQPLinkSendFlowScheduler} to send a flow frame to the
     * link receiver asking for link credit if the link credit < 0 and it has
     * outstanding messages to send.
     *
     * @param currentTime
     */
    void requestCreditIfUnderFlowControl(long currentTime) {
        ...
        synchronized (this) {
            if ((linkCredit > 0) || (unsentMessages.isEmpty())) {
                /*
                 * Is not under flow-control anymore, or doesn't need
                 * link-credit. Reset nextCreditRequestTime.
                 */
                nextCreditRequestTime = -1;
                incrementedCount = 0;
                return;
            ...
            /*
             * A flow-control condition was detected. Schedule the sending of
             * flow-frame to until after 2 seconds.
             */
            ...
            /*
             * Next credit request time is still in the future, so just return.
             */
            ...
            /*
             * The subsequent flow-frame sending is scheduled with an
             * exponential back-off (in seconds) : 2, 4, 8, 16, 32 After that it
             * is scheduled every 60 seconds.
             */
    ...
    public void messageSent(CAMQPControlTransfer transferFrame) {
        ...
        synchronized (this) {
            ...
            if (!sendInProgress) {
                if (canSendMessage()) {
                    /*
                     * Messages are waiting at the Link layer to be sent,
                     * throttled by a session-level flow-control. Send those
                     * messages now that we have enough link credit.
                     */
                    sendInProgress = true;
                    parkedMessages = true;
                }
    ...
    /**
     * This Runnable runs as a Link sender, and sends all the unsent messages,
     * until it runs out of Link credit. If it is done sending messages, and if
     * there's an outstanding drain requested, it sends a flow frame.
     */
    @Override
    public void run() {
    ...
    /**
     * Returns true if there are outstanding messages to be sent, and there is
     * enough credit. If there are outstanding unsent messages at the session
     * layer, the Link layer acts as if it does not have the link credit.
     *
     * @return
     */
    @GuardedBy("this")
    ...
    void registerWithFlowScheduler() {
        CAMQPLinkManager.getLinkmanager()
                .getFlowScheduler()
                .registerLinkSender(linkHandle, this);
    }

    void unregisterWithFlowScheduler() {
        CAMQPLinkManager.getLinkmanager()
                .getFlowScheduler()
                .unregisterLinkSender(linkHandle);
    }

    /**
     * Establishes an AMQP link to a remote AMQP end-point. Then waits for the
     * first non-zero link-credit.
     *
     * @param sourceName
     * @param targetName
     */
