...
@ThreadSafe
final class CAMQPLinkReceiver extends CAMQPLinkEndpoint implements CAMQPLinkReceiverInterface {
    ...
    /*
     * Used to record the time the last flow-frame was sent with linkCredit
     * boost for CREDIT_STEADY_STATE_DRIVEN_BY_TARGET_MESSAGE_PROCESSING.
     */
    private long timeLastFlowFrameSent = System.currentTimeMillis();

    /*
     * In case of CREDIT_STEADY_STATE_DRIVEN_BY_TARGET_MESSAGE_PROCESSING, the
     * following attribute keeps track of how many messages have been processed
     * by LinkTarget since the last flow frame was issued. The sender is then
     * given a link-credit boost for this amount, the next time a flow frame is
     * sent.
     */
    ...
    public void transferReceived(long transferId, CAMQPControlTransfer transferFrame, CAMQPMessagePayload payload) {
        /*
         * In case of message fragmentation, change the I/O flow state and make
         * link-layer flow-control calculations only when the last fragment has
         * been received.
         */
        ...
        synchronized (this) {
            ...
            if ((linkCredit < 0) && (-linkCredit > CAMQPLinkConstants.LINK_CREDIT_VIOLATION_LIMIT)) {
                ...
                log.fatal("violated link credit: closing link. Link credit should not have gone below -" +
                CAMQPLinkConstants.LINK_CREDIT_VIOLATION_LIMIT + " but is now " + linkCredit);
            }
            else {
                /*
                 * To keep the flow at steady state, automatically boost the
                 * link credit and send a flow-frame to the Link sender, if the
                 * link credit drops below minLinkCreditThreshold.
                 */
        ...
        /*
         * Deliver the message to the Link target. TODO reassemble transfer
         * frames of a fragmented message.
         */
        deliverMessage(transferFrame, payload);
        /*
         * Acknowledge the receipt of transfer frame to the Session layer.
         */
    ...
    /**
     * Processes an incoming Link flow-frame: (a) updates flow-control
     * attributes. (b) Gives linkCredit to sender based on policy and flow
     * frame. (c) Sends back a flow frame if needed.
     */
    @Override
    public void flowReceived(CAMQPControlFlow inFlow) {
        ...
        synchronized (this) {
            ...
            /*
             * Link sender has messages available, but has run out of link
             * credit. Issue link credit based on the policy.
             */
    ...
    /**
     * Called by target to alter linkCredit of receiver. Note that the
     * linkCredit is not changed right away. The reason is that the Link sender
     * might be sending messages with the last known link credit, and if we
     * suddenly reduce the linkCredit of the receiver, the sender might appear
     * to overrun the link credit, if it does not get the new flow frame until
     * it has sent enough messages to appear to have breached the link credit.
     * So, we store the newLinkCredit in a member attribute
     * targetIssuedLinkCredit. The linkCredit is altered only after it has been
     * exhausted. In other words, the sender is provided with a new link credit
     * only after it has used up its current linkCredit.
     *
     * @param newLinkCredit
     * @param drain
     */
    ...
    /**
     * Called by Link target to asynchronously receive messages. Offers a link
     * credit that is equal to the expected message count. Also sets the drain
     * flag to true, so that the Link sender advances deliveryCount, even if
     * enough messages are not available.
     */
    @Override
    ...
    /**
     * Called by Link target to configure the steady-state link credit policy.
     * It also immediately boosts the link credit if necessary. Subsequently,
     * whenever the link sender's link credit drops below the configurable
     * threshold, the link credit is boosted, and a flow frame generated.
     */
    @Override
    ...
    /**
     * Called by Link target to stop receiving messages on the link, by dropping
     * the link credit to 0. Note that, the LinkReceiver might still receive
     * messages that are on the flight, or are sent by the LinkSender before
     * this flow frame is processed.
     */
    @Override
    public void stop() {
        CAMQPControlFlow flow = null;
    ...
    /**
     * Called by target end-point upon processing of a message. Applicable only
     * for
     * {@link ReceiverLinkCreditPolicy#CREDIT_STEADY_STATE_DRIVEN_BY_TARGET_MESSAGE_PROCESSING}
     * Increments messagesProcessedSinceLastSendFlow. Boosts link-credit and
     * sends a flow-frame if (a) a flow frame hasn't been sent for a while or
     * (b) messagesProcessedSinceLastSendFlow exceeds
     * linkCreditBoost-minLinkCreditThreshold
     */
    @Override
    ...
    /**
     * Configure the link credit state based on CAMQPEndpointPolicy. Send a
     * flow-frame to the peer if needed.
     */
    @Override
