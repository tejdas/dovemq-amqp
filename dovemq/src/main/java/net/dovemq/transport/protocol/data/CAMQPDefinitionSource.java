...
public class CAMQPDefinitionSource
{
    public static final String descriptor = "amqp:source:list";
    ...
    private boolean isSetDistributionMode = false;
    public void setRequiredDistributionMode(boolean val)
    {
        isSetDistributionMode = val;
    }
    ...
    public void setDistributionMode(String val)
    {
        isSetDistributionMode = true;
    ...
    public void setRequiredFilter(boolean val)
    {
        isSetFilter = val;
    }
    ...
    private boolean isSetDefaultOutcome = false;
    public void setRequiredDefaultOutcome(boolean val)
    {
        isSetDefaultOutcome = val;
    }
    public boolean isSetDefaultOutcome()
    {
        return isSetDefaultOutcome;
    }
    private Object defaultOutcome = null;
    public void setDefaultOutcome(Object val)
    {
        isSetDefaultOutcome = true;
        defaultOutcome = val;
    }
    ...
    private boolean isSetOutcomes = false;
    public void setRequiredOutcomes(boolean val)
    {
        isSetOutcomes = val;
    }
    ...
    private Collection<String> outcomes = new ArrayList<String>();
    public void addOutcomes(String val)
    {
        isSetOutcomes = true;
        outcomes.add(val);
    }
    ...
    public static void encode(CAMQPEncoder encoder, CAMQPDefinitionSource data)
    {
        long listSize = 12;
        ...
        if ((data.distributionMode != null) && (data.isSetDistributionMode))
        {
            encoder.writeSymbol(data.distributionMode);
        }
        else
        {
            encoder.writeNull();
        ...
        if ((data.filter != null) && (data.filter.size() > 0) && (data.isSetFilter))
        {
            int size = data.filter.size();
            encoder.writeMapHeader(size);
            Set<Entry<String, String>> entries = data.filter.entrySet();
            for (Entry<String, String> entry : entries)
            {
                encoder.writeSymbol(entry.getKey());
                encoder.writeUTF8String(entry.getValue());
        ...
        if ((data.defaultOutcome != null) && (data.isSetDefaultOutcome))
        {
            if (data.defaultOutcome instanceof CAMQPDefinitionModified)
            {
                CAMQPDefinitionModified.encode(encoder, (CAMQPDefinitionModified) data.defaultOutcome);
            }
            else if (data.defaultOutcome instanceof CAMQPDefinitionReleased)
            {
                CAMQPDefinitionReleased.encode(encoder, (CAMQPDefinitionReleased) data.defaultOutcome);
            }
            else if (data.defaultOutcome instanceof CAMQPDefinitionAccepted)
            {
                CAMQPDefinitionAccepted.encode(encoder, (CAMQPDefinitionAccepted) data.defaultOutcome);
            }
            else if (data.defaultOutcome instanceof CAMQPDefinitionRejected)
            {
                CAMQPDefinitionRejected.encode(encoder, (CAMQPDefinitionRejected) data.defaultOutcome);
            }
        }
        else
        {
        ...
        if ((data.outcomes != null) && (data.isSetOutcomes))
        {
            long outcomesSize = data.outcomes.size();
            if (outcomesSize > 0)
            {
                encoder.writeArrayHeaderForMultiple(outcomesSize, CAMQPFormatCodes.SYM8);
                for (String val : data.outcomes)
                {
                    encoder.writeSymbolArrayElement(val);
                }
                encoder.fillCompoundSize(outcomesSize);
            }
            else
            {
                encoder.writeNull();
    ...
    public static CAMQPDefinitionSource decode(CAMQPSyncDecoder decoder)
    {
        int formatCode;
        ...
        if (formatCode != CAMQPFormatCodes.NULL)
        {
            data.distributionMode = decoder.readString(formatCode);
            data.isSetDistributionMode = true;
        }
        ...
        if (formatCode != CAMQPFormatCodes.NULL)
        {
            ...
            for (long innerIndex = 0; innerIndex < innerMapSize; innerIndex++)
            {
                ...
                if ((innerKey != null) && (innerVal != null))
                {
                    data.filter.put(innerKey, innerVal);
                }
            }
            data.isSetFilter = true;
        }
        ...
        if (decoder.isNextDescribedConstructor())
        {
            ...
            {
                data.defaultOutcome = CAMQPDefinitionModified.decode(decoder);
            }
            else if (controlName.equalsIgnoreCase(CAMQPDefinitionReleased.descriptor))
            {
                data.defaultOutcome = CAMQPDefinitionReleased.decode(decoder);
            }
            else if (controlName.equalsIgnoreCase(CAMQPDefinitionAccepted.descriptor))
            {
                data.defaultOutcome = CAMQPDefinitionAccepted.decode(decoder);
            }
            else if (controlName.equalsIgnoreCase(CAMQPDefinitionRejected.descriptor))
            {
                data.defaultOutcome = CAMQPDefinitionRejected.decode(decoder);
            }
        }
        else
        {
...

        {
            ...
            {
                data.outcomes.add(decoder.readString(compoundHeader.elementFormatCode));
            }
        }
