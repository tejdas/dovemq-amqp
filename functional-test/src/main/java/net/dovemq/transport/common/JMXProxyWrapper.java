...
public class JMXProxyWrapper {
    ...
    public JMXProxyWrapper(String remoteIp, String jmxPort) throws IOException,
            MalformedObjectNameException {
        JMXServiceURL url = new JMXServiceURL(String.format("service:jmx:rmi:///jndi/rmi://%s:%s/jmxrmi",
                remoteIp,
                jmxPort));
        ...
        sessionBean = JMX.newMBeanProxy(mbsc,
                mbeanName,
                SessionCommandMBean.class,
                true);

        ObjectName mbeanName2 = new ObjectName(JMXConstants.CONNECTION_COMMAND_BEAN);
        connectionBean = JMX.newMBeanProxy(mbsc,
                mbeanName2,
                ConnectionCommandMBean.class,
                true);

        ObjectName mbeanName3 = new ObjectName(JMXConstants.LINK_COMMAND_BEAN);
        linkBean = JMX.newMBeanProxy(mbsc,
                mbeanName3,
                LinkCommandMBean.class,
                true);
    }
